...
test("Home → add a tag → download → tag shows in Library", async () => {
  ...
  try {
    ...
    await expect(window.locator('[data-testid="tag-chip"][data-tag="e2etag"]')).toBeVisible();
    ...
    await expect(window.locator('[data-testid="tag-chip"][data-tag="e2etag"]').first()).toBeVisible();
  } finally {
    await app.close();
