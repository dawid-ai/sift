...
test("library paginates: result count, numbered pages, prev/next, jump, page size", async () => {
  ...
  try {
    ...
    execFileSync(electronBinaryPath as unknown as string, [join(__dirname, "seed-many.cjs"), join(userDataPath, "sift.db"), "130"], {
    ...
    await expect(window.getByTestId("library-result-count")).toHaveText("Showing 1–24 of 130");
    ...
    await expect(window.getByTestId("library-result-count")).toHaveText("Showing 25–48 of 130");
    await expect(window.getByTestId("library-page-prev")).toBeEnabled();
    await expect(window.getByTestId("library-page-2")).toHaveAttribute("aria-current", "page");
    ...
    await expect(window.getByTestId("library-result-count")).toHaveText("Showing 121–130 of 130");
    ...
    await expect(window.getByTestId("library-result-count")).toHaveText("Showing 1–130 of 130");
