...
describe("resolveAssetPath", () => {
  ...
  it("joins a nested relative path (whisper)", () => {
    expect(resolveAssetPath(BASE, "whisper\\whisper-cli")).toBe(
...
describe("normalizeAssetPaths", () => {
  ...
  it("rewrites an absolute-under-binariesDir row to relative", async () => {
    ...
    expect(
      db
        .prepare<{ path: string }>("SELECT path FROM asset WHERE kind='ytdlp'")
        .get()!.path,
    ).toBe("yt-dlp.exe");
  });
  it("rewrites a nested absolute path to nested relative", async () => {
    const db = await seedDb([
      { kind: "whisper", path: `${BASE}\\whisper\\whisper-cli` },
    ]);
    normalizeAssetPaths(db, BASE);
    expect(
      db
        .prepare<{ path: string }>(
          "SELECT path FROM asset WHERE kind='whisper'",
        )
        .get()!.path,
  ...
  it("leaves an already-relative row untouched (idempotent)", async () => {
    ...
    expect(
      db
        .prepare<{ path: string }>("SELECT path FROM asset WHERE kind='ffmpeg'")
        .get()!.path,
    ).toBe("ffmpeg.exe");
  });
  it("leaves an absolute path OUTSIDE binariesDir untouched (e.g. homebrew)", async () => {
    const db = await seedDb([
      { kind: "whisper", path: "/opt/homebrew/bin/whisper-cli" },
    ]);
    normalizeAssetPaths(db, BASE);
    expect(
      db
