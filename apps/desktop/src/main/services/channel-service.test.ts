...
import { runMigrations, insertMedia, insertDownload, insertQueueItem, upsertChannel, upsertSubscription, listSubscriptions, type SiftDatabase } from "@sift/db";
import { ChannelService, type ChannelServiceDeps } from "./channel-service";

function channelRaw(entries: { id: string; view_count?: number; duration?: number }[]) {
  return {
    channel_id: "UC1", channel: "Chan", uploader_id: "@chan",
    channel_follower_count: 10, playlist_count: 100, thumbnails: [],
    entries: entries.map((e) => ({ ...e, url: `https://www.youtube.com/watch?v=${e.id}`, title: e.id })),
  };
}

function makeDeps(db: SiftDatabase, flat: (url: string, opts: { items?: string }) => Promise<unknown>): ChannelServiceDeps {
...
describe("ChannelService", () => {
  ...
  it("videoStatuses flags downloaded and queued videos, leaves the rest absent", async () => {
    ...
    const m = insertMedia(db, {
      source_url: dl, platform_id: "youtube", external_id: "DL", title: "t", uploader: null,
      uploader_url: null, duration_s: null, thumbnail_path: null, view_count: null, like_count: null,
      published_at: null, metadata_json: null, download_status: "done",
    });
    insertDownload(db, {
      media_id: m.id, format_id: "137", label: "1080p", ext: "mp4", height: 1080,
      file_path: "/x.mp4", file_size: null, status: "done", error: null,
    });
    insertQueueItem(db, { source_url: q, spec_json: "{}", status: "queued", ops_json: null, media_id: null, queue_order: 1, error: null });
  ...
  it("add upserts by channel_id (same channel twice → one row)", async () => {
    const svc = new ChannelService(makeDeps(db, async () => channelRaw([{ id: "v1" }])));
  ...
  it("refresh computes new_count since last_seen and advances it", async () => {
    let entries = [{ id: "v1" }];
    const svc = new ChannelService(makeDeps(db, async () => channelRaw(entries)));
  ...
  it("re-adding an already-tracked channel preserves N-new tracking, refreshes display", async () => {
    ...
    raw = { ...channelRaw([{ id: "v3" }, { id: "v2" }, { id: "v1" }]), channel: "Old Title" };
    ...
    raw = { ...channelRaw([{ id: "v4" }, { id: "v3" }, { id: "v2" }, { id: "v1" }]), channel: "New Title" };
  ...
  it("listVideos latest requests items 1:count", async () => {
    ...
    flat.mockClear();
    const res = await svc.listVideos(c.id, { contentType: "videos", order: "latest", count: 2 });
    expect(flat).toHaveBeenCalledWith(expect.stringContaining("/videos"), { items: "1:2" }, undefined);
  ...
  it("listVideos live requests items 1:count with /streams tab", async () => {
    ...
    const c = await svc.add("https://youtube.com/@chan");
    flat.mockClear();
    const res = await svc.listVideos(c.id, { contentType: "live", order: "latest", count: 2 });
    expect(flat).toHaveBeenCalledWith(expect.stringContaining("/streams"), { items: "1:2" }, undefined);
    expect(res.videos.map((v) => v.externalId)).toEqual(["v2", "v1"]);
  });
  ...
  it("listVideos oldest requests items -count: and reverses", async () => {
    ...
    const c = await svc.add("https://youtube.com/@chan");
    flat.mockClear();
    const res = await svc.listVideos(c.id, { contentType: "videos", order: "oldest", count: 2 });
    expect(flat).toHaveBeenCalledWith(expect.anything(), { items: "-2:" }, undefined);
  ...
  it("most_viewed sorts by views; falls back to latest with a flag when no views", async () => {
    const withViews = channelRaw([{ id: "a", view_count: 5 }, { id: "b", view_count: 99 }, { id: "c", view_count: 50 }]);
    const svc = new ChannelService({ db, runner: { flatPlaylist: vi.fn(async () => withViews) } });
    const c = await svc.add("https://youtube.com/@chan");
    const res = await svc.listVideos(c.id, { contentType: "videos", order: "most_viewed", count: 2 });
    ...
    const svc2 = new ChannelService({ db, runner: { flatPlaylist: vi.fn(async () => noViews) } });
    const c2 = await svc2.add("https://youtube.com/@chan");
    const res2 = await svc2.listVideos(c2.id, { contentType: "videos", order: "most_viewed", count: 2 });
  ...
  it("refreshAll: one failing channel does not abort the rest; failures reported", async () => {
    ...
    const ch2 = upsertChannel(db, {
      channel_id: "UCchan2", url: "https://www.youtube.com/channel/UCchan2/chan2", handle: "@chan2",
      title: "Chan2", description: null, uploader: null, avatar_url: null, banner_url: null,
      follower_count: null, video_count: null, last_seen_video_id: null, new_count: 0, last_checked: null,
    });
    const ch3 = upsertChannel(db, {
      channel_id: "UCchan3", url: "https://www.youtube.com/channel/UCchan3", handle: "@chan3",
      title: "Chan3", description: null, uploader: null, avatar_url: null, banner_url: null,
      follower_count: null, video_count: null, last_seen_video_id: null, new_count: 0, last_checked: null,
    });

    const res = await svc.refreshAll();
    expect(res.refreshed.map((c) => c.channelId).sort()).toEqual([ch1.channelId, ch3.channel_id].sort());
  ...
  it("channelUrlForMedia reads channel_url from the media's metadata_json, falls back to uploader_url", async () => {
    const m = insertMedia(db, {
      source_url: "https://youtube.com/watch?v=x", platform_id: "youtube", external_id: "x",
      title: "T", uploader: "U", uploader_url: "https://youtube.com/@fallback",
      duration_s: null, thumbnail_path: null, view_count: null, like_count: null,
      published_at: null, metadata_json: JSON.stringify({ channel_url: "https://youtube.com/channel/UCZ" }),
      download_status: "none",
    });
    const svc = new ChannelService(makeDeps(db, async () => channelRaw([{ id: "v1" }])));
    expect(await svc.channelUrlForMedia(m.id)).toBe("https://youtube.com/channel/UCZ");
    const m2 = insertMedia(db, {
      source_url: "https://youtube.com/watch?v=y", platform_id: "youtube", external_id: "y",
      title: "T", uploader: "U", uploader_url: "https://youtube.com/@fallback",
      duration_s: null, thumbnail_path: null, view_count: null, like_count: null,
      published_at: null, metadata_json: JSON.stringify({}), download_status: "none",
    });
    expect(await svc.channelUrlForMedia(m2.id)).toBe("https://youtube.com/@fallback");
  });
});

describe("subscriptions", () => {
  it("syncs feed/channels, persists, and tags tracked against My channels", async () => {
    const db = await openTestDatabase(); runMigrations(db);
    // A channel already in "My channels" → its sub row must come back tracked:true.
    upsertChannel(db, {
      channel_id: "UCaaa", url: "https://www.youtube.com/channel/UCaaa", handle: "@a",
      title: "Alpha", description: null, uploader: null, avatar_url: null, banner_url: null,
      follower_count: null, video_count: null, last_seen_video_id: null, new_count: 0, last_checked: null,
    });
    const runner = {
      flatPlaylist: async (url: string) => {
        ...
        return {
          entries: [
            ...
            { id: "UCbbb", channel: "Bravo", uploader_id: "b", channel_follower_count: 9 },
          ],
        };
      },
    };
  ...
  it("an empty feed/channels fetch does not wipe the previously-synced subscription list", async () => {
    ...
    upsertSubscription(db, {
      channel_id: "UCaaa", url: "https://www.youtube.com/channel/UCaaa", handle: "@a",
      title: "Alpha", avatar_url: null, follower_count: null, synced_at: Date.now(),
    });
