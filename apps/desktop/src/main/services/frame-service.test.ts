...
import { FrameService, settledGrabTimes, type FrameProgress } from "./frame-service";
import type { OcrResult, OcrRunner } from "../sidecars/ocr";

function media(db: SiftDatabase): number {
  const m: NewMedia = {
    source_url: "https://y/1", platform_id: "youtube", external_id: "abc", title: "Talk",
...
const slide = (text: string): OcrResult => ({ text, wordCount: text.split(/\s+/).length, meanConfidence: 90 });
...
function fakeFfmpeg(sceneTimes: number[]) {
  return {
    ...
    extractFrameAt: vi.fn(async ({ outputPath }: { outputPath: string }) => writeFileSync(outputPath, "x")),
  };
}
...
describe("FrameService.extract", () => {
  it("grabs settled frames, keeps slides, drops scenery (row + file), closes OCR", async () => {
    ...
    const [f1, f2, f3] = ["frame-0001.jpg", "frame-0002.jpg", "frame-0003.jpg"].map((n) => join(dir, n));
    ...
    const ocr: OcrRunner = { recognize: vi.fn(async (p: string) => byPath[p]!), close };

    const progress: FrameProgress[] = [];
    const service = new FrameService({ db, ffmpeg, makeOcr: () => ocr, framesDir: () => dir });
    const kept = await service.extract({ mediaId: mid, videoPath: "/v.mp4" }, (p) => progress.push(p));
    ...
    expect(progress.at(-1)).toEqual({ stage: "done", ratio: 1, processed: 3, total: 3, kept: 2 });
  });
  ...
  it("drops a frame that perceptually repeats an already-kept slide (skips its OCR)", async () => {
    ...
    const mid = media(db);
    const dir = mkdtempSync(join(tmpdir(), "sift-frames-"));
    const [, f2, f3] = ["frame-0001.jpg", "frame-0002.jpg", "frame-0003.jpg"].map((n) => join(dir, n));
    ...
    const ocr: OcrRunner = { recognize, close: vi.fn().mockResolvedValue(undefined) };

    const service = new FrameService({ db, ffmpeg, makeOcr: () => ocr, framesDir: () => dir, hashFrame });
  ...
  it("drops frames the AI classifier rejects (before OCR)", async () => {
    ...
    const mid = media(db);
    const dir = mkdtempSync(join(tmpdir(), "sift-frames-"));
    const [f1, f2] = ["frame-0001.jpg", "frame-0002.jpg"].map((n) => join(dir, n));

    const ffmpeg = fakeFfmpeg([20]); // midpoints 10 (f1), 60 (f2)
    const recognize = vi.fn(async () => slide("Real slide with plenty of text"));
    const ocr: OcrRunner = { recognize, close: vi.fn().mockResolvedValue(undefined) };
    const classify = vi.fn(async (p: string) => p === f1); // only f1 is a slide

    const service = new FrameService({ db, ffmpeg, makeOcr: () => ocr, framesDir: () => dir });
    const kept = await service.extract({ mediaId: mid, videoPath: "/v.mp4", classifier: { classify } });
  ...
  it("fullScreenOnly drops dark room shots, keeps bright full-screen slides", async () => {
    ...
    const service = new FrameService({ db, ffmpeg, makeOcr: () => ocr, framesDir: () => dir, brightFraction });
    const kept = await service.extract({ mediaId: mid, videoPath: "/v.mp4", fullScreenOnly: true });
  ...
  it("passes the crop through to each grabbed frame", async () => {
    ...
    expect(ffmpeg.extractFrameAt).toHaveBeenCalledWith(expect.objectContaining({ crop }));
  });
  ...
  it("captureFrame grabs one frame at a timestamp, OCRs it, stores it as manual", async () => {
    ...
    const row = await service.captureFrame({ mediaId: mid, videoPath: "/v.mp4", tsMs: 42_000 });
    ...
    expect(ffmpeg.extractFrameAt).toHaveBeenCalledWith(
      expect.objectContaining({ seconds: 42, outputPath: join(dir, "manual-42000.jpg") }),
    );
  ...
  it("re-extract keeps manual captures but replaces auto frames", async () => {
    ...
    const service = new FrameService({ db, ffmpeg, makeOcr, framesDir: () => dir });

    await service.captureFrame({ mediaId: mid, videoPath: "/v.mp4", tsMs: 5000 });
