...
describe("serveThumb", () => {
  let dir: string;
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "sift-thumb-"));
  });
  afterEach(() => {
  ...
  it("rejects a non-allowlisted host without fetching", async () => {
    ...
    expect(
      await serveThumb({ dir, fetchImpl: f }, "https://evil.example.com/x.jpg"),
    ).toBeNull();
  ...
  it("rejects a malformed url", async () => {
    expect(
      await serveThumb({ dir, fetchImpl: fakeFetch(JPEG) }, "not a url"),
    ).toBeNull();
  });

  it("downloads on miss, caches to disk, and sniffs the jpeg type", async () => {
    const f = fakeFetch(JPEG);
    const r = await serveThumb(
      { dir, fetchImpl: f },
  ...
  it("returns null on a non-2xx response", async () => {
    expect(
      await serveThumb(
        { dir, fetchImpl: fakeFetch(JPEG, false) },
