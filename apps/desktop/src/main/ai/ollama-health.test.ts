...
describe("ollamaReachable", () => {
  ...
  it("true when the ping responds ok", async () => {
    const fake = (async () => ({ ok: true }) as Response) as unknown as typeof fetch;
    expect(await ollamaReachable(url, fake)).toBe(true);
  });
  it("false on a non-ok response", async () => {
    const fake = (async () => ({ ok: false }) as Response) as unknown as typeof fetch;
...
describe("startOllama", () => {
  it("reports not-installed when spawn emits an error", async () => {
    const fakeSpawn = () => {
      ...
      return { on: (ev: string, cb: (e: Error) => void) => (handlers[ev] = cb), unref: () => {} };
    };
    expect(await startOllama(fakeSpawn)).toEqual({ launched: false, reason: "not-installed" });
  });
