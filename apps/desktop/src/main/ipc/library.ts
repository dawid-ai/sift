...
export function registerLibraryIpc(service: DownloadService): void {
  ...
  ipcMain.handle(IPC.libraryListPage, (_e, filter: MediaFilter, page: number, pageSize: number) =>
    service.listPage(filter, page, pageSize),
  );
  ipcMain.handle(IPC.libraryFacets, () => service.facets());
  ipcMain.handle(IPC.libraryListIds, (_e, filter: MediaFilter) => service.listIds(filter));
