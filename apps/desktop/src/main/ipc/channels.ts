...
export function registerChannelsIpc(service: ChannelService): void {
  ...
  ipcMain.handle(IPC.channelListVideos, (_e, id: number, query: ChannelVideosQuery) => service.listVideos(id, query));
  ipcMain.handle(IPC.channelOpenForMedia, (_e, mediaId: number) => service.openForMedia(mediaId));
  ipcMain.handle(IPC.channelVideoStatuses, (_e, urls: string[]) => service.videoStatuses(urls));
  ipcMain.handle(IPC.channelDownloadedMedia, (_e, channelId: string) => service.downloadedMedia(channelId));
