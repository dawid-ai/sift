...
describe("updateReducer", () => {
  ...
  it("checking → available → downloading advances percent → downloaded", () => {
    ...
    s = updateReducer(s, { type: "available", version: "1.2.3", releaseNotes: "notes" });
    expect(s).toEqual({ kind: "available", version: "1.2.3", releaseNotes: "notes" });
  ...
  it("ignores late download-progress once downloaded", () => {
    ...
    expect(updateReducer(downloaded, { type: "downloading", percent: 99 })).toEqual(downloaded);
  });

  it("dismiss returns to idle from any state", () => {
    const avail = { kind: "available", version: "1", releaseNotes: "" } as const;
    expect(updateReducer(avail, { type: "dismiss" })).toEqual({ kind: "idle" });
  });

  it("maps not-available and error", () => {
    expect(updateReducer(initialUpdateState, { type: "not-available" })).toEqual({ kind: "not-available" });
    expect(updateReducer(initialUpdateState, { type: "error", message: "boom" })).toEqual({
      kind: "error",
      message: "boom",
    });
  });
