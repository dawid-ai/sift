...
describe("externalLinkUrl", () => {
  it("passes http(s) through", () => {
    expect(externalLinkUrl("https://x.com/someone")).toBe(
      "https://x.com/someone",
    );
    expect(externalLinkUrl("HTTP://vimeo.com/user")).toBe(
      "HTTP://vimeo.com/user",
    );
  });
...
describe("mediaFileUrl", () => {
  it("encodes the path into the sift-media protocol", () => {
    expect(mediaFileUrl("C:\\v\\a b.mp4")).toBe(
      "sift-media://file/C%3A%5Cv%5Ca%20b.mp4",
    );
  });
});
...
describe("videoThumbUrl", () => {
  it("routes YouTube-CDN thumbnails through the sift-thumb cache", () => {
    ...
    expect(videoThumbUrl("https://yt3.ggpht.com/x")).toContain(
      "sift-thumb://img/",
    );
  });
  ...
  it("routes an imported file's local poster path through sift-poster", () => {
    ...
    expect(
      videoThumbUrl("C:\\Users\\d\\AppData\\Roaming\\Sift\\posters\\7.jpg"),
    ).toBe(
