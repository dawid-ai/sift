...
export function SubscriptionsTab({
  ...
  onImport: (sub: SubscriptionRecord) => void;
}) {
  ...
  useEffect(() => {
    void window.sift.subscriptions.list().then(setSubs);
  }, []);
  ...
  const sync = async () => {
    ...
    setError(null);
    try {
      setSubs(await window.sift.subscriptions.sync());
    } catch (e) {
  ...
  const shown = subs.filter((s) => {
    ...
    return (
      ...
      (s.handle ?? "").toLowerCase().includes(t)
    );
  });
    ...
    >
      <section className="panel-lit px-6 py-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Button
            data-testid="subscriptions-sync"
          ...
          {shown.map((s) => {
            const meta = [
              ...
              s.followerCount != null
                ? `${s.followerCount.toLocaleString()} subs`
                : null,
            ]
              .filter(Boolean)
              .join(" · ");
