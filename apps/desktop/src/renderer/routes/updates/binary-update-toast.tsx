...
import { AlertTriangle, ArrowRight, ArrowUpCircle, CheckCircle2, DownloadCloud } from "lucide-react";
 ...
 */
function VersionPill({ children, next = false }: { children: string; next?: boolean }) {
  return (
    <span className={chipClass(next ? "accent" : "neutral", "font-mono tabular-nums tracking-normal")}>
      {children}
    </span>
  );
}
    ...
    >
      {notices.map((n) => {
        ...
        const tone: ToastTone =
          n.type === "ready" ? "success" : n.type === "error" ? "danger" : "primary";
        return (
          <div
            key={n.kind}
          ...
          >
            ...
            {n.type === "ready" && (
              <>
                <ToastHead
                  tone="success"
                  label={n.reason === "installed" ? `${label} installed` : `Updated ${label}`}
                ...
                <div className={TOAST_ACTIONS}>
                  <Button size="sm" variant="outline" onClick={() => onDismiss(n.kind)}>
                    Dismiss
                  </Button>
                </div>
              </>
              ...
              <>
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <ArrowRight className="h-3.5 w-3.5 flex-none text-foreground/45" aria-hidden />
            ...
            {n.type === "error" && (
              <>
                ...
                >
                  <p className="text-[13px] leading-relaxed text-muted-foreground">{n.message}</p>
                </ToastHead>
                <div className={TOAST_ACTIONS}>
                  <Button size="sm" variant="outline" onClick={() => onDismiss(n.kind)}>
                    Dismiss
