...
function addMedia(
  ...
  const overrides: Partial<NewMedia> = {
    source_url: over.url ?? `https://y/${Math.random()}`,
  };
...
describe("listPlaylistEntries", () => {
  ...
  it("excludes media with no done download", () => {
    ...
    expect(listPlaylistEntries(db, [a, b, c]).map((e) => e.mediaId)).toEqual([
      a,
    ]);
  });
  ...
  it("picks the newest done download when a media has several", () => {
    ...
    addDownload(db, a, {
      formatId: "137",
      filePath: "C:/dl/old.mp4",
      status: "done",
    });
    addDownload(db, a, {
      formatId: "248",
      filePath: "C:/dl/new.mp4",
      status: "done",
    });
