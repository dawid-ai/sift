...
export function insertFrame(db: SiftDatabase, f: NewFrame): FrameRow {
  ...
  const res = db
    ...
    .run({ ...f, created_at });
  return db
    .prepare<FrameRow>("SELECT * FROM frame WHERE id = @id")
    .get({ id: Number(res.lastInsertRowid) })!;
}

/** Frames for a media, ordered along the timeline (oldest timestamp first). */
export function getFramesByMediaId(
  db: SiftDatabase,
  mediaId: number,
): FrameRow[] {
  return db
    .prepare<FrameRow>(
      "SELECT * FROM frame WHERE media_id = @mediaId ORDER BY ts_ms ASC, id ASC",
    )
    .all({ mediaId });
}
...
export function deleteAutoFramesByMediaId(
  db: SiftDatabase,
  mediaId: number,
): void {
  db.prepare(
    "DELETE FROM frame WHERE media_id = ? AND kind IS NOT 'manual'",
  ).run(mediaId);
}
...
export function setFrameIncluded(
  ...
  db.prepare("UPDATE frame SET included = ? WHERE id = ?").run(
    included ? 1 : 0,
    id,
  );
}
...
export function frameExistsByImagePath(
  db: SiftDatabase,
  imagePath: string,
): boolean {
  return (
    db.prepare("SELECT 1 FROM frame WHERE image_path = ?").get(imagePath) !==
    undefined
  );
}
