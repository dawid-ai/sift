...
describe("sql.js test driver adapter", () => {
  it("handles DDL, named + positional params, upsert, and PRAGMA", async () => {
    ...
    db.exec(`CREATE TABLE t (id INTEGER PRIMARY KEY AUTOINCREMENT, kind TEXT UNIQUE, n INTEGER)`);

    // named params (@name style, like better-sqlite3)
    db.prepare(`INSERT INTO t (kind, n) VALUES (@kind, @n)`).run({ kind: "a", n: 1 });
    ...
    const row = db.prepare<{ kind: string; n: number }>("SELECT kind, n FROM t WHERE kind = ?").get("a");
    expect(row?.n).toBe(2);
    ...
    const cols = db.prepare<{ name: string }>("PRAGMA table_info(t)").all().map((c) => c.name);
