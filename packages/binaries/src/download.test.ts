...
function fakeFetch(): typeof fetch {
  return (async () =>
    new Response(bytes, { status: 200, headers: { "content-length": String(bytes.length) } })) as unknown as typeof fetch;
}
...
describe("downloadAndVerify", () => {
  it("writes the file when the sha256 matches", async () => {
    ...
    await downloadAndVerify({ url: "https://x/y", destPath: dest, expectedSha256: goodSha, fetchImpl: fakeFetch() });
  ...
  it("throws and leaves no file when the sha256 mismatches", async () => {
    ...
    const dest = join(dir, "yt-dlp");
    await expect(
      downloadAndVerify({ url: "https://x/y", destPath: dest, expectedSha256: "deadbeef", fetchImpl: fakeFetch() }),
  ...
  it("throws and writes nothing on an HTTP error response", async () => {
    ...
    await expect(
      downloadAndVerify({ url: "https://x/y", destPath: dest, expectedSha256: goodSha, fetchImpl: notFound }),
    ).rejects.toThrow(/404/);
    expect(existsSync(dest)).toBe(false);
