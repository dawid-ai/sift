...
describe("resolveDeno", () => {
  it("resolves every platform with a pinned version, a github assetUrl, a 64-hex sha, and the right binary name", () => {
    for (const p of PLATFORMS) {
      ...
      expect(r.assetUrl).toContain(
        `/denoland/deno/releases/download/${DENO_VERSION}/`,
      );
  ...
  it("uses the msvc asset for windows and the correct triple per platform", () => {
    expect(resolveDeno("mac-arm64").assetUrl).toContain(
      "deno-aarch64-apple-darwin.zip",
    );
    expect(resolveDeno("linux-x64").assetUrl).toContain(
      "deno-x86_64-unknown-linux-gnu.zip",
    );
    expect(resolveDeno("win-x64").assetUrl).toContain(
